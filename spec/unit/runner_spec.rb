...
describe Runner do
  ...
  let(:packages) do
    [{:name => "miku", :file_contents => "miku 1.1-dev", :version => "1.1-dev"},
     {:name => "luca", :file_contents => "tenshi", :version => "1"}]
  end
  ...
  def check_installed_job_files
    ...
    expect_contents(install_dir, "monit", "job", "0000_#{success_job}.angel.monitrc").to eq("monit mode manual")
  end
  ...
  context "archive mode" do
    ...
    it "should create job archive" do
      ...
      out = %x[bundle exec ./bin/nise-bosh -y -a -d #{install_dir} --working-dir #{working_dir} #{release_dir} #{deploy_conf} #{success_job} 2>&1]
      expect($?.exitstatus).to eq(0)
    ...
    it "should create job archive in given directory with default file name" do
      out = %x[bundle exec ./bin/nise-bosh -y -a -d #{install_dir} --working-dir #{working_dir} #{release_dir} #{deploy_conf} #{success_job} #{archive_dir} 2>&1]
      expect($?.exitstatus).to eq(0)
    ...
    it "should create job archive with given file name " do
      ...
      out = %x[bundle exec ./bin/nise-bosh -y -a -d #{install_dir} --working-dir #{working_dir} #{release_dir} #{deploy_conf} #{success_job} #{archive_name} 2>&1]
      expect($?.exitstatus).to eq(0)
